export const fetchFarms = async (lat: number, lon: number, cant?: number) => {
  ...
  const apiUrl = cant
    ? `http://192.168.0.38:3000/farmacias-abiertas-o-de-turno?lat=${lat}&lon=${lon}&cantidad=${cant}`
    : `http://192.168.0.38:3000/farmacias-abiertas-o-de-turno?lat=${lat}&lon=${lon}`;
